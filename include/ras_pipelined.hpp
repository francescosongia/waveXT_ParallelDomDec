...
template<class P>
class RasPipelined : public DomainDecSolverBase<P> {
public:
  RasPipelined(Domain dom, const Decomposition& dec, LocalMatrices local_matrices,const SolverTraits& traits) : 
        DomainDecSolverBase<P>(dom,dec,local_matrices,traits),solves_(0),subt_sx_(1),zone_(2),it_waited_(0), matrix_domain_(dec.nsub_t(),dec.nsub_x())
        {
  ...
  SolverResults solve(const SpMat& A, const SpMat& b) override
    {
        ...
        SolverResults res_obj(uw,solves_,time, this->traits_, this->DataDD);
...
class PipeParallel : public RasPipelined<PipeParallel>
{
  public:
    PipeParallel(Domain dom, const Decomposition& dec,const LocalMatrices local_matrices, const SolverTraits& traits) : 
    RasPipelined<PipeParallel>(dom,dec,local_matrices,traits) {};

    Eigen::VectorXd precondAction(const SpMat& x)
    {
      int rank = this->local_mat.rank();
      int np = this->local_mat.sub_assignment().np();
      if(this->DataDD.nsub_x() % np != 0){
...
class PipeSequential : public RasPipelined<PipeSequential>
{
  public:
    PipeSequential(Domain dom, const Decomposition& dec,const LocalMatrices local_matrices,const SolverTraits& traits) :
     RasPipelined<PipeSequential>(dom,dec,local_matrices,traits) {};
